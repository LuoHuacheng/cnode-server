...
class Topics {
  async getAllTopics (req, res, next) {
    ...
    try {
      const allTopics = await TopicsModel.find(query, { _id: 0 }).skip((page - 1) * limit).limit(parseInt(limit)).sort({create_at: -1})
      res.send({
        code: 1,
        message: 'ok',
        data: allTopics
  ...
  async postOneTopic (req, res, next) {
    ...
    const { login_name, avatar_url, author_id } = await UserModel.findOne({ author_id: req.session.token_id })
    const topic = Object.assign(req.body, { author: { login_name, avatar_url }, author_id })
    try {
  ...
  async getOneTopic (req, res, next) {
    ...
    try {
      ...
      await TopicsModel.findOneAndUpdate({ id }, { visit_count: data.visit_count += 1 })
      res.send({
        msg: 'ok',
        code: 1,
        data
